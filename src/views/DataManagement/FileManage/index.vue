...
<!--            />-->
    ...
    <template v-slot:crudAction>
      <el-button @click="uploadTip">专利情报分析</el-button>
    </template>
    <template v-slot:table>
      ...
      <el-table-column label="完成时间" prop="prompt" align="center" />
...
export default {
  ...
  methods: {
    ...
    async handleNodeClick(data) {
      if (this.currentTreeId === data.id) return;
      // this.resetTree();
    ...
    async getInitData() {
      // 获取可选年份下拉
    //   await getAcademic(this.currentTreeId).then((response) => {
    //     this.yearOption = response.data.map((item) => {
    //       let yearArea = "";
    //       if (this.activeTab == "school") {
    //         yearArea = `${item.academicYear} - ${parseInt(item.academicYear) + 1}`;
    //       } else {
    //         yearArea = item.academicYear;
    //       }
    //       return { ...item, yearArea };
    //     });
    //   });
    },
  }
