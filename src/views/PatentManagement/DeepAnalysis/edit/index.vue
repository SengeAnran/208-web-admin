...
<!--          </el-col>-->
        ...
        <div class="flex" style="margin-bottom: 45px">
          ...
          <div class="patent">
            <div class="patent-item" v-for="(item, index) in info.patentNames" @click="goDetailPage(item)" :key="index">
              {{ item.name }}
            </div>
          </div>
        </div>
...
import {editOne, exportSingle, getDetail, reAnalysis} from "@/api/DeepAnalysis.js";
...
export default {
  ...
  methods: {
    ...
    // 保存
    handleSave() {
      const {id, basicOverview, technicalSituation, inspirationAndSuggestion, title, tips} = this.info;
      console.log(basicOverview, technicalSituation, inspirationAndSuggestion);
      const data = {
        id, basicOverview, technicalSituation, inspirationAndSuggestion, title,
    ...
    //  导出
    expertData() {
      exportSingle({id: this.info.id}).then(res => {
        downloadWordFile(res,  this.info.title);
      })
    },
  }
}
