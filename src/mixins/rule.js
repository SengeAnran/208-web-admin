...
const patentRule = (rule, value, callback) => {
    if (value.length === 0) {
        callback(new Error("请添加分类并添加对应专利"));
    } else if (value.length === 1 && value[0].patentCategory === '') {
        callback(new Error("请添加分类并添加对应专利"));
    }else if (!isAllLength5(value)) {
        callback(new Error("每个分类最少一个专利最多五个专利"));
    } else {
        callback();
    }
};
...
function isAllLength5(arr) {
    return !(arr.some(i => {
        return i.patentCategory === '' || i.patents.length > 5 || (i.patents.length === 0)
    }))
}
/**
 * 所有数组元素都有内容
